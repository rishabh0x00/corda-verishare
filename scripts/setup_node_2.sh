...
docker-compose -f docker-compose.node-2.yml exec postgres-2 psql -U postgres -c "CREATE DATABASE ${databaseName};"

#Start Backend services
echo "-----------------Starting Backend services----------------------"
make start-backend-2

## Run create table migration
echo "-------------------Running Migrations-----------------"
docker-compose -f docker-compose.node-2.yml exec backend-2 typeorm migration:run
...
curl -s -X POST http://localhost:8082/auth/admin/realms \
-H "Authorization: Bearer $admin_token" \
-H 'content-type: application/json' \
-d "{\"id\": \"$(uuidgen)\",\"realm\": \"grepruby\", \"enabled\": true}"

secret=$(uuidgen)
base_url=$(echo ${input_details} | jq -r '.baseUrl')
#Redirect uri
curl -s -X POST http://localhost:8082/auth/admin/realms/grepruby/clients \
-H "Authorization: Bearer $admin_token" \
-H 'content-type: application/json' \
...
client_id=$(curl -s -X GET http://localhost:8082/auth/admin/realms/grepruby/clients \
-H "Authorization: Bearer $admin_token" | jq -r '.[] | select(.clientId=="react") | .id')

curl -s -X POST "http://localhost:8082/auth/admin/realms/grepruby/roles" \
-H "Authorization: Bearer $admin_token" \
-H 'content-type: application/json' \
-d '{"name": "org-employee"}'
...
curl -s -X POST "http://localhost:8082/auth/admin/realms/grepruby/users" \
-H "Authorization: Bearer $admin_token" \
-H 'content-type: application/json' \
...
admin_keycloak_id=$(curl -s -X GET "http://localhost:8082/auth/admin/realms/grepruby/users" \
-H "Authorization: Bearer $admin_token" | jq -r '.[0] | .id')

role_id=$(curl -s -X GET "http://localhost:8082/auth/admin/realms/grepruby/roles/org-admin" \
-H "Authorization: Bearer $admin_token" | jq -r '.id')


curl -s -X POST "http://localhost:8082/auth/admin/realms/grepruby/users/$admin_keycloak_id/role-mappings/realm" \
-H "Authorization: Bearer $admin_token" \
-H 'content-type: application/json' \
-d "[{\"name\": \"org-admin\", \"id\": \"$role_id\"}]"

service_account_keycloak_id=$(curl -s -X GET "http://localhost:8082/auth/admin/realms/grepruby/clients/$client_id/service-account-user" \
-H "Authorization: Bearer $admin_token" | jq -r '.id')

realm_id=$(curl -s -X GET http://localhost:8082/auth/admin/realms/grepruby \
-H "Authorization: Bearer $admin_token" | jq -r '.id')
...
echo ${details} > org2Details.json


## Create organization in blockchain
# echo "-----------------------Creating Organization in Blockchain-----------------------"
# organization_Info=$(curl -s -X POST 'http://localhost:3002/api/v1/organizations' \
# -H "Content-Type: application/json" \
